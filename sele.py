...
for a in l:
    ...
    for b in range(nm):
        ...
        time.sleep(5)
        it=[{'data-zop':eval(dehtml(c.split('data-zop="')[1].split('"')[0]).replace('true','True').replace('false','False').replace('null','None'))}if('//www.zhihu.com/question/'not in c)else{'data-zop':eval(dehtml(c.split('data-zop="')[1].split('"')[0]).replace('true','True').replace('false','False').replace('null','None')),'link':'https://www.zhihu.com/question/%s'%c.split('//www.zhihu.com/question/')[1].split('"')[0]}for c in br.page_source.split('<div class="Pagination">')[0].split('<div class="jsNavigable CollectionDetailPageItem css-vurnku">')[1:]]
        it2=[]
